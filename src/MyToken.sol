...
contract MyToken is ECR20{
    ...
    function mintTokenswithMTC(address recipient) external {
        _mint(recipient, 10000000 * (10 ** 18)); 
    ...
    function approveLendContract(address spender, uint256 amount) external returns (bool) {
        ...
        require(amount <= allowance(msg.sender, spender), "Amount exceeds balance"); 

        _aprrove(msg.sender, spender, amount);
